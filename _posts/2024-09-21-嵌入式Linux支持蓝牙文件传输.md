...
**如果选择不移植`bluealsa`，启动`bluetoothd`的时候禁用掉`a2dp`也可以实现文件传输。**
```shell
bluetoothd --noplugin=a2dp --plugin=avrcp -n &
```
...
要支持手机主动连接，需要打开`discoverable on`和`default-agent`。
```shell
power on
agent on
discoverable on
...
6. [BLUEZ5.5蓝牙发送接收文件](https://blog.csdn.net/ldinvicible/article/details/131851316)
